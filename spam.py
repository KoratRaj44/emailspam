...
def creds_enterd():
	if st.session_state["user"].strip() == "admin" and  st.session_state["passwd"].strip() =="admin":
		...
		if not st.session_state["passwd"]:
			...
			st.warning("please entar username.")
		else:
			st.error("Invalid Username/Password :face_with_raised_eyebrow:")

def authenticated_user():
	if "authenticated" not in st.session_state:
		st.text_input(label="Username :",value="", key="user", on_change=creds_enterd)
		st.text_input(label="Password :",value="", key="passwd", type="password", on_change=creds_enterd)
		return False
	else:
            ...
            if entities == 'no':
              st.subheader('No Named Entities found.')
                ...
                for i in renames.keys():
                    if i in entities:  # Check if the key exists in the dictionary
                       with st.expander(renames[i]):
                           ...
                           st.write(f"No entities of type '{renames[i]}' found.")
